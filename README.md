...
Version 1.7.2 is the current release. Existing 1.x imports, selectors, data hooks, ARIA hooks, and tokens remain supported.
...
The equivalent unpkg URL is `https://unpkg.com/interactive-surface-css@1.7.2/standalone-preset.css`. To follow future releases deliberately, use `https://cdn.jsdelivr.net/npm/interactive-surface-css@latest/standalone-preset.css` — unpinned opt-in.
...
`standalone-preset.css` and `interactive-surface.css` are generated from the same authored modules and are behaviorally equivalent in 1.7.2. The compatibility paths remain stable; no 1.x migration is required.
