...
Version 1.7.1 is a release candidate until it is published. It preserves every established 1.x import and selector while ensuring consumer-owned positioned controls remain authoritative beside the focused `state-core.css` and complete `standalone-preset.css` entry points.
