...
Version 1.7.0 is published on npm. It preserves every established 1.x import and selector while adding CSS-only `data-surface-feedback="error|success|attention"` outcomes to the focused `state-core.css` and complete `standalone-preset.css` entry points.
