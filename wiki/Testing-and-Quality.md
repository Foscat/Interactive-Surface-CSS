...
The 1.7.2 release uses layered gates so contributors can choose fast deterministic checks or the complete cross-browser suite without confusing the two.
...
The exact script graph is finalized as part of the 1.7.2 release before publication. Browser downloads intentionally remain outside `prepublishOnly`.
