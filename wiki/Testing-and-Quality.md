...
The 1.7.0 release uses layered gates so contributors can choose fast deterministic checks or the complete cross-browser suite without confusing the two.
...
The exact script graph is finalized for the 1.7.0 release. Browser downloads intentionally remain outside `prepublishOnly`.
