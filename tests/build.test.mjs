...
test("the bundle map exposes every public and distribution stylesheet", async () => {
  ...
  const publicKeys = Object.keys(bundleMap).filter(
  ...
  publicBundleNames.forEach((bundleName) => {
    ...
    assert.ok(
      bundleMap[`dist/${stem}.min.css`],
      `Missing minified bundle: ${stem}.min.css`,
    );
  });
  ...
  ["standalone-preset.css", "interactive-surface.css"].forEach((bundleName) => {
    const exactBaseSelectors =
      bundleMap[bundleName].match(/^\.interactive-surface \{/gm) ?? [];

    assert.equal(
      exactBaseSelectors.length,
      1,
      `${bundleName} contains a duplicate base selector`,
    );
  });
});
...
test("bundle parity reports the stale target without mutating project files", async (t) => {
  const outputDirectory = await mkdtemp(
    path.join(tmpdir(), "interactive-surface-build-"),
  );
  const bundleMap = await createBundleMap();
  ...
  for (const [target, expected] of Object.entries(bundleMap)) {
    assert.equal(
      await readFile(path.join(outputDirectory, target), "utf8"),
      expected,
    );
  }
  ...
  await writeFile(
    path.join(outputDirectory, crlfTarget),
    bundleMap[crlfTarget].replaceAll("\n", "\r\n"),
  );
  await verifyBundles(bundleMap, outputDirectory);

  const staleTarget = "interactive-surface.css";
  await writeFile(
    path.join(outputDirectory, staleTarget),
    "/* stale fixture */\n",
  );

  await assert.rejects(
    verifyBundles(bundleMap, outputDirectory),
...
test("public parity can run before ignored distribution bundles exist", async (t) => {
  const outputDirectory = await mkdtemp(
    path.join(tmpdir(), "interactive-surface-public-build-"),
  );
  const bundleMap = await createBundleMap();
